...
feature
	...
	test_binary_search_alt
			-- Test of routine `binary_search_alt'.
		local
			a: SIMPLE_ARRAY [INTEGER]
			s: BINARY_SEARCH
			i: INTEGER
		do
			...
			i := s.binary_search_alt (a, 2)
            check a.sequence.has (2) end
			check i = 5 end
			check a[i] = 2 end

			i := s.binary_search_alt (a, 3)
			check not a.sequence.has (3) end
			check i = 0 end

			i := s.binary_search_alt (a, -5)
            check a.sequence.has (-5) end
			check i = 1 end
			check a[i] = -5 end

			i := s.binary_search_alt (a, 17)
			check not a.sequence.has (17) end
			check i = 0 end
		end
